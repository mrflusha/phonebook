...
class edit_contact_w(QDialog):                           # <===
    ...
    def edit_w(self):
        ...
        edit_layout.setColumnStretch(4, 3)
        ...
        self.contact_name.setPlaceholderText("Имя контакта")
        self.phone.setPlaceholderText("Номер телефона")
        self.date_label = QLabel("Дата рождения:")
        #self.calendar = QCalendarWidget()
        #self.date_c.setMaximumDateTime()

        self.edit_button.setStyleSheet('QPushButton {background-color: green; color: white;cursor:pointer}')
        ...
        edit_layout.addWidget(self.contact_name,0,1,1,3)
        edit_layout.addWidget(self.phone,1,1,1,3)

        edit_layout.addWidget(self.date_label,4,1,1,3)
        edit_layout.addWidget(self.date_c,5,1,1,3)
        edit_layout.addWidget(self.edit_button,6,1)
        edit_layout.addWidget(self.cancel_button,6,2)
        self.horizontalGroupBox.setLayout(edit_layout)
        print(self.date_c)
        self.cancel_button.clicked.connect(self.cancel_click)
        self.edit_button.clicked.connect(self.create_click)
    ...
    def create_click(self):
        ...
        print(date,str(date_str.date())+" \ntype of:"+str(type(date_str)))
        



class create_contact_w(QDialog):                           # <===
    def __init__(self,parent=None):
        super().__init__(parent, Qt.Window)
    ...
    def create_w(self):
        ...
        create_layout.setColumnStretch(4, 3)
        self.contact_name = QLineEdit(self)
        self.phone = QLineEdit(self)
        ...
        self.create_button.setStyleSheet('QPushButton {background-color: green; color: white;cursor:pointer}')
        self.create_button.setCursor(QCursor(Qt.PointingHandCursor))
        self.cancel_button.setStyleSheet('QPushButton {background-color: red; color: white;}')
        self.cancel_button.setCursor(QCursor(Qt.PointingHandCursor))

        create_layout.addWidget(self.contact_name,0,1,1,3)
        create_layout.addWidget(self.phone_label,1,0)
        create_layout.addWidget(self.phone,1,1,1,3)

        create_layout.addWidget(self.date_label,4,1,1,3)
        create_layout.addWidget(self.date_c,5,1,1,3)
        create_layout.addWidget(self.create_button,6,1)
        create_layout.addWidget(self.cancel_button,6,2)
        self.horizontalGroupBox.setLayout(create_layout)
        print(self.date_c)
        self.cancel_button.clicked.connect(self.cancel_click)
    ...
    def cancel_click(self):
            self.close()
    def create_click(self):
 #       user_id = getattr(App, createGridLayout.user_id)
        print(user_id)
        ...
        try:
            num = int(num)
            db_func.add_contact(str(user_id),name,num,date)
        except Exception:
...
class output_window(QDialog):                           # <===
    ...
    def createOutputWindow(self):
        ...
        self.table.setRowCount(len(db_func.butt_id))
        self.table.setHorizontalHeaderLabels(["ФИО", "Номер телефона", "Дата рождения"])
        self.table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.table.setSelectionMode(QAbstractItemView.SingleSelection)
        self.table.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.tables_arr = list()
        
        for i in range(len(db_func.butt_name)):
                #print(db_func.td[j])
            print(db_func.butt_name.index('helli'))
            print(db_func.butt_name[i])
            self.table.setItem(i,0, QTableWidgetItem(db_func.butt_name[i]))
            self.table.setItem(i,1, QTableWidgetItem(db_func.butt_phone[i]))
            self.table.setItem(i,2, QTableWidgetItem(db_func.butt_bd[i]))
        ...
        self.add_button.clicked.connect(self.create_contact)
        self.edit_button.clicked.connect(self.edit_contact)
        self.tmp = list()
        for j in range(len(self.buttons_arr)):
            self.tmp.append(int(j))
            print(self.tmp)
            self.buttons_arr[j].clicked.connect(self.buttons_filter)
    ...
    def create_contact(self):
        self.w = create_contact_w()
        self.w.show()

    def edit_contact(self):
        self.w = edit_contact_w()
        self.w.show()
...
#Get button styles
    ...
    def open_output(self):
        auth_login = self.login.text()
        auth_pw = self.password.text()
        global user_id


        user_id = db_func.auth(db_func.auth_q,auth_login,auth_pw)
        if user_id != False:
            self.w = output_window()
            self.w.show()
            self.close()
        else:
            QMessageBox.warning(self, "Уведомление", "Введённые данные некоректны", QMessageBox.Ok)   
